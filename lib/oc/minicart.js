 ...
 *
 * Every price here is gated on BOTH the HidePricing permission and the order actually
 * having a total. The permission alone was not enough: a Uniform Champion does not carry
 * HidePricing — they buy a la carte and genuinely need to see prices — so an empty cart
 * rendered as " - $0.00" beside the cart icon on every page. Allocation shoppers were
 * covered only because that permission happens to be set for them, which made the bug
 * look like it belonged to champions rather than to the condition.
 *
 * Same rule as the cart and checkout summaries: money appears when there is money. A
 * champion with something chargeable in the cart still sees its total.
 */
...
function minicart() {
    ...
    function template(){
        return [
            '<style>',
            '.minicart {float: right; cursor: pointer;color: #242629; min-width:300px; text-align:right;padding-top: 0;margin-right: 30%;}',
            ...
            '    <div class="minicart">',
            '        <i class="fa fa-shopping-cart" style="color:rgb(0, 38, 62);"></i> <span class="label label-default hidden">{{cartCount}}</span> <span ng-if="!(user.Permissions.contains(\'HidePricing\')) && currentOrder.Total > 0" class="hidden-xs" style="color:rgb(0, 38, 62);"> - {{currentOrder.Total | currency }}</span> <i class="fa fa-caret-down text-muted" style="color:rgb(0, 38, 62);"></i>',
            ...
            '                        <div class="col-xs-3">',
            '                            <a ng-href="{{\'cart/\' + lineitem.Product.InteropID + \'/\' + $index}}" style="pointer-events: none;cursor: default;text-decoration: none;">',
            '                                <figure>',
            ...
            '                                </div>',
            '                                <div class="col-xs-3" ng-if="!(user.Permissions.contains(\'HidePricing\')) && currentOrder.Total > 0">',
            ...
            '<li>',
            '            </ul>',
            '            <div class="row" ng-if="!(user.Permissions.contains(\'HidePricing\')) && currentOrder.Total > 0">',
            ...
            '            </div>',
            '            <div class="row" ng-if="!(user.Permissions.contains(\'HidePricing\')) && currentOrder.Total > 0 && currentOrder.TaxCost">',

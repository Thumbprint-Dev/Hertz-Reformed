...
function largeshipaddresssearch() {
    ...
    function template() {
        // Hidden when there is exactly one address to ship to.
        //
        // LargeShipAddressSearchCtrl sets `readonlyshipping` when the list has one entry,
        // picks it, and makes the box read-only — which is every Hertz employee, because
        // they ship to their department. The result was a disabled search field repeating,
        // as one run-on string, the address already set out properly in the summary just
        // below it, under a tip telling them to type a location code they have no reason
        // to type. `ng-hide`, not `ng-if`: the input carries `required`, and it has to stay
        // in the form to keep satisfying it.
        return [
            '<style>',
            '.largeaddress .dropdown-menu { background-color:#fff; }',
            '.largeaddress .dropdown-menu .active > a { background-color:#11172b !important; color:#fff; }',
            '</style>',
            '<div class="largeaddress hz-co-field" ng-hide="readonlyshipping">',
            '<label class="hz-co-label is-required">{{("Shipping" | r) + " " + ("Address" | r) | xlat}}</label>',
            '<p class="hz-co-hint" ng-show="showTip">Start typing your location number (area or department code) to find your address.</p>',
            '<p class="hz-co-hint hz-co-hint--bad" ng-show="showResult">No addresses found.</p>',
            '<div>',
            '<input class="hz-co-input" type="text" ng-readonly="readonlyshipping" ng-model="ShipAddress" required ng-change="searchShipAddresses(ShipAddress)" typeahead-min-length="3" typeahead="address as (address.AddressName + \' \' + (address.FirstName || \'\') + \' \' + (address.LastName || \'\') + \' \' + (address.Street1 || \'\') + \' \' + (address.Street2 || \'\') + \' \' + (address.City || \'\') + \' \' + (address.State || \'\') + \' \' + (address.Zip || \'\')) for address in shipaddresses | filter:$viewValue | limitTo:10" />',
            '</div>',
            '</div>'
        ].join('');
    }

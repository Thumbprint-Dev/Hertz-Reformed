four51.app.factory('ProductDisplayService', ['$sce', '$451', 'Variant', 'Product', function($sce, $451, Variant, Product){
    ...
    var variantSpecNames = [];
    angular.forEach(scope.LineItem.Specs, function(s){
        if(s.DefinesVariant){
            variantSpecNames.push(s.Name.toLowerCase());
        }
    });

    var errorMsg;
    if(variantSpecNames.length === 1 && variantSpecNames[0] === "color"){
        errorMsg = "The color that you selected is not available, please choose another color.";
    } else if(variantSpecNames.length === 1 && variantSpecNames[0] === "size"){
        errorMsg = "The size that you selected is not available, please choose another size.";
    } else if(variantSpecNames.length === 2 && variantSpecNames.indexOf("color") !== -1 && variantSpecNames.indexOf("size") !== -1){
        errorMsg = "The color and size combination that you selected is not available, please choose another.";
    } else {
        errorMsg = "This item is not available, please choose another item.";
    }

    newErrorList.push(errorMsg);
}
